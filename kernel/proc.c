...
int clone(void* stack,int size)
{
  ...
  *p->tf = *curproc->tf;
  ...
  p->tf->eax = 0;
  ThreadNum++;
  void *var1 = (void*)curproc->tf->ebp +16;
  void *var2 = (void*)curproc->tf->esp;
  uint sz1 = (uint)(var1 - var2);
  p->tf->esp = (uint) (stack - sz1);
  p->tf->ebp = (uint) (stack - 16);
  memmove(stack-sz1,var2,sz1);
  for(i=0;i<NOFILE;i++)
  {
    if(curproc->ofile[i])
    {
